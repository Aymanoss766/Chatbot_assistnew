...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="bg-primary px-6 py-10" data-testid="header-section">
              ...
              {!statusLoading && (
                <span
                  className={`inline-block h-3 w-3 rounded-full ${
                    allConfigured ? "bg-green-400" : someConfigured ? "bg-yellow-400" : "bg-red-400"
                  }`}
                  data-testid="status-indicator"
                />
              )}
            </div>
            <p className="mt-2 text-primary-foreground/80 text-sm" data-testid="text-subtitle">
              {status?.openRouterModel
                ? `Active Model: ${status.openRouterModel}`
                : "Powered by OpenRouter - StepFun Step 3.5 Flash"}
              {status && status.pagesCount > 0 && (
                <span className="ml-3 inline-flex items-center gap-1">
                  <SiFacebook className="h-3 w-3" />
                  {status.pagesCount} {status.pagesCount === 1 ? "page" : "pages"} connected
                </span>
              )}
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        >
          <h2 className="text-lg font-semibold mb-4" data-testid="text-config-heading">Configuration Status</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statusLoading ? (
              ...
              <>
                <Card data-testid="card-verify-token">
                ...
                <Card data-testid="card-page-access-token">
                  <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
                ...
                <Card data-testid="card-openrouter-key">
                  <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">OpenRouter API Key</CardTitle>
        ...
        >
          <Card data-testid="card-webhook-url">
            ...
            <CardContent>
              <div className="flex items-center gap-2 flex-wrap">
                <code className="flex-1 min-w-0 truncate rounded-md bg-muted px-3 py-2 text-sm font-mono" data-testid="text-webhook-url">
                  {webhookUrl}
                </code>
        ...
        >
          <Card className="border-2 border-blue-200 dark:border-blue-900 overflow-hidden" data-testid="card-facebook-credentials">
        ...
        >
          <Card className="border-2 border-green-200 dark:border-green-900 overflow-hidden" data-testid="card-whatsapp">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="ai-model" type="text" placeholder="stepfun/step-3.5-flash:free" value={aiModel} onChange={(e) => setAiModel(e.target.value)} data-testid="input-ai-model" />
                  <p className="text-xs text-muted-foreground">Leave empty to use the default model. Browse models at openrouter.ai/models</p>
                </div>
                <Button onClick={() => {
                  if (!aiApiKey.trim()) { toast({ title: "Missing API Key", description: "Please enter an OpenRouter API key.", variant: "destructive" }); return; }
                  aiMutation.mutate({ apiKey: aiApiKey.trim(), model: aiModel.trim() });
        ...
        >
          <Card data-testid="card-image-config">
            <CardHeader>
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
